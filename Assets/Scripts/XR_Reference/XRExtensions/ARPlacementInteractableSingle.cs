...
public class ARPlacementInteractableSingle : ARBaseGestureInteractable
{
    ...
    {                        
        /*
        if(placementObject.tag == "Toilet" || placementObject.tag == "Table")
        {
            GWorld.Instance.GetQueue(foData.resourceQueue).RemoveResource(placementObject);
    ...
    public void ActivateToilet()
    {
        ...
        placementPrefab = allResources[0];
    }
    ...
    public void ActivateTable()
    {
        ...
        placementPrefab = allResources[1];
    }
    ...
    protected override void OnEndManipulation(TapGesture gesture)
    {
        ...
        // arCamera?????ray???
        /*
        var ray = arCamera.ScreenPointToRay(gesture.StartPosition);
        Resource r = rhit.transform.gameObject.GetComponent<Resource>();
        bool hasHit = Physics.Raycast(ray, out rhit);
        ...
        if (GestureTransformationUtility.Raycast(gesture.StartPosition, hits, TrackableType.PlaneWithinPolygon))
        {
            ...
            else if (placementPrefab.tag == "Table" || placementPrefab.tag == "Toilet")
            {
                placementObject = Instantiate(placementPrefab, newPlacePos, hit.pose.rotation);
                //foData = placementObject.GetComponent<Resource>().info;
            }
            else
            {
            ...
            else if (placementObject.tag == "Toilet" || placementObject.tag == "Table")
            {
                anchorObject.transform.position = newPlacePos;
                //foData = r.info;
            }
            ...
            if (placementObject.tag == "Toilet" || placementObject.tag == "Table")
            {
                GWorld.Instance.GetQueue(foData.resourceQueue).AddResource(placementObject);
                GWorld.Instance.GetWorld().ModifyState(foData.resourceState, 1);
            }*/
